...
- **多切片批处理**：用户约定「一批 / 连续 / 多垂直切片」时，按 **`docs/vertical-slices.md`** 与既有黄金/链式测试模式**默认不中断**——不每步询问「下一步做哪个」，直至本批 **`make spine`**（必要时 **`make verify`**）与 **`Makefile` / 文档计数**同步收束；用户若要求逐步确认则另当别论。  
