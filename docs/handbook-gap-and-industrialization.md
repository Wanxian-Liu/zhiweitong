...
- **代码**：**`make spine`** + **`make verify`**（或 CI 绿）；岗位以 **`org_canonical`** 为准，未进简化版的不算。  
- **上线实践**：代码绿之后，在目标环境按 **`docs/ops-runbook.md`** 配好变量与总线，**跑通一轮演示**即可；细节随项目再补，**不必**先做完本文件里全部工业级 P0。  
- **上线后**：运维里迭代；**新岗**先改本节与 **`org_canonical`**。
