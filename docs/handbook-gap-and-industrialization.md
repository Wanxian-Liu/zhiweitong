...
| **L2** | **可对账**：与台账/报表 **同粒度** 可核对；异常码、人工兜底与 Runbook 段落齐备 | **`production-inventory-v1`**、**`production-quality-v1`**、**`finance-ar-ap-v1`**、**`warehouse-cycle-transfer-v1`** 等切片链上叶岗已达 L2（`summary.l2_reconcile` 与 **`docs/ops-runbook.md`**）；其余岗继续按切片抬升 |
