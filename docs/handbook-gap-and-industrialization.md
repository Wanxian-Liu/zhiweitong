...
1. **开发封版（可证）**  
   - 上表 **「已实现」** 行：每一叶岗 **`ORG_PATH`** 与 **`shared/org_canonical.py`** 一致；**`make spine`** + **`make verify`** 通过（与 **`.github/workflows/ci.yml`** 一致）。  
   - **「未实现 / 待定」** 行：**不**计入第一步完成度，直至大纲定岗并落地代码与注册。

2. **上线实践（发版侧）**  
   - 在目标环境跑通约定演示或小流量（部署、密钥、总线后端等见 **`docs/ops-runbook.md`**、**`docs/event_topics.md`**）。本仓库**不替代**你们的发版清单，但**以第一步清单为范围边界**。

3. **运维迭代（上线后）**  
   - 按监控与工单纠错、补文档；**新需求若新增岗位**，须回到本节与 **`org_canonical`** 走「先大纲、后代码」，避免 silent 扩 scope。

**与手册 Phase 0–3 的关系**：第一步清单是 **城市乳业业务封版子集**；手册中带 **「—」** 的技术行仍按上文「封版与持续迭代」执行。二者同时满足时，**第一步 + 手册封版** 可并行宣称完成。
