...
- **`exception_code`**：`W_MRP_NET_SHORTAGE`（原料净需求大于现存量）、`W_INBOUND_SHORTFALL`（订购与实收短缺）、`W_OUTBOUND_SHORTFALL`（出库需求与实拣短缺）、`I_REORDER_SUGGESTED`（现存量低于阈值，信息类）；**生产补链**（质检→放行）：`W_QC_BATCH_REJECT`（缺陷超阈值）、`W_RELEASE_BLOCKED`（未质检放行）；**仓储补链**（盘点→调拨）：`W_CYCLE_COUNT_VARIANCE`（账存与实盘不一致）、`W_TRANSFER_SHORTFALL`（调拨量大于源库位可用量）；**财务岗**：`W_FIN_AR_LINE_MISMATCH`、`W_FIN_AP_LINE_MISMATCH`（应收/应付与收付款笔数不一致的示意规则）；正常为 `null`。  
