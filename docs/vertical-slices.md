...
L1 黄金 JSON（happy + **`boundary/`** 行数不一致 → **`W_FIN_AR_LINE_MISMATCH`** / **`W_FIN_AP_LINE_MISMATCH`**）：`tests/fixtures/golden/finance_ar_ap_v1/`、`tests/test_zz_golden_finance_ar_ap_v1_json.py`。
...
L1 黄金 JSON（happy + **`boundary/`** 试算不平衡 → **`W_TRIAL_IMBALANCE`**、试算未过 → **`W_FIN_REPORT_BLOCKED`**）：`tests/fixtures/golden/finance_trial_report_v1/`、`tests/test_zz_golden_finance_trial_report_v1_json.py`。
...
L1 黄金 JSON（happy + **`boundary/`** 超缺陷阈值 → **`W_QC_BATCH_REJECT`**、质检未过 → **`W_RELEASE_BLOCKED`**）：`tests/fixtures/golden/production_quality_v1/`、`tests/test_zz_golden_production_quality_v1_json.py`。
...
L1 黄金 JSON（happy + **`boundary/`** 账实差异 → **`W_CYCLE_COUNT_VARIANCE`**、源量不足 → **`W_TRANSFER_SHORTFALL`**）：`tests/fixtures/golden/warehouse_cycle_transfer_v1/`、`tests/test_zz_golden_warehouse_cycle_transfer_v1_json.py`。
