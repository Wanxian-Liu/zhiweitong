"""L1 黄金用例：财务补链 ``finance-trial-report-v1``（试算平衡 → 报表快照）脱敏 JSON + 沙盒断言。

与 ``shared.vertical_slices.FINANCE_TRIAL_REPORT_DEFAULT_PARAMS`` 及
``tests/test_zz_vertical_slice_finance_trial_report_chain.py`` 一致。
...
_FIXTURE_DIR = Path(__file__).resolve().parent / "fixtures" / "golden" / "finance_trial_report_v1"
_COVERAGE_MODULES: tuple[str, ...] = (
    "skills.finance_center.trial_balance",
    "skills.finance_center.report_snapshot",
)
...
def test_golden_fin_tr_fixture_params_match_vertical_slice_defaults() -> None:
    ...
    for i, p in enumerate(paths):
        ...
        assert case["params"] == FINANCE_TRIAL_REPORT_DEFAULT_PARAMS[i], p.name
...
def test_golden_fin_tr_sandbox_matches_fixture(fixture_path: Path) -> None:
...
def test_golden_fin_tr_boundary_sandbox_matches_fixture(fixture_path: Path) -> None:
    case = json.loads(fixture_path.read_text(encoding="utf-8"))
    ...
    async def _run() -> None:
        ...
        if ex in ("W_TRIAL_IMBALANCE", "W_FIN_REPORT_BLOCKED"):
