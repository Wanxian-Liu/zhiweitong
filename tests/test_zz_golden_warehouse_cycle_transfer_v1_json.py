"""L1 黄金用例：仓储补链 ``warehouse-cycle-transfer-v1``（盘点 → 调拨）脱敏 JSON + 沙盒断言。

与 ``shared.vertical_slices.WH_CYCLE_TRANSFER_DEFAULT_PARAMS`` 及
``tests/test_zz_vertical_slice_wh_cycle_transfer_chain.py`` 一致。
...
_FIXTURE_DIR = Path(__file__).resolve().parent / "fixtures" / "golden" / "warehouse_cycle_transfer_v1"
...
def test_golden_wh_ct_fixture_params_match_vertical_slice_defaults() -> None:
    ...
    for i, p in enumerate(paths):
        ...
        assert case["params"] == WH_CYCLE_TRANSFER_DEFAULT_PARAMS[i], p.name
...
def test_golden_wh_ct_sandbox_matches_fixture(fixture_path: Path) -> None:
...
def test_golden_wh_ct_boundary_sandbox_matches_fixture(fixture_path: Path) -> None:
    case = json.loads(fixture_path.read_text(encoding="utf-8"))
    ...
    async def _run() -> None:
        ...
        if ex in ("W_CYCLE_COUNT_VARIANCE", "W_TRANSFER_SHORTFALL"):
